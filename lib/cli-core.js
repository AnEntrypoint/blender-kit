...
function registerCoreCommands(program) {
  ...
  function pyTool(tool, label, okMsg) {
    return (files) => {
      const targets = files.length ? files : ['.'];
      try {
        const r = execSync([tool, ...targets].join(' '), { stdio: 'pipe', encoding: 'utf8' });
        if (r) console.log(r);
        console.log(okMsg);
      } catch (e) {
        if (isToolNotFound(e)) { console.log(`${tool} not found. Run: blender-dev setup`); return; }
  ...
  program.command('lint [files...]').description('Lint Python files using flake8').action(pyTool('flake8', 'lint', '\x1b[32mLint passed.\x1b[0m'));
  program.command('format [files...]').description('Format Python files using black').action(pyTool('black', 'format', '\x1b[32mFormat complete.\x1b[0m'));
  ...
  program.command('install-addon').description('Write Blender bridge addon to ./addons/blender_bridge/__init__.py')
    .option('-a, --auto', 'also copy directly to Blender scripts/addons folder')
    .action((opts) => {
      const addonSrc = path.join(__dirname, '..', 'addons', 'blender_bridge', '__init__.py');
      const addonDest = path.join(process.cwd(), 'addons', 'blender_bridge', '__init__.py');
      fs.mkdirSync(path.dirname(addonDest), { recursive: true });
      fs.copyFileSync(addonSrc, addonDest);
      const dir = path.dirname(addonDest);
      console.log(`\x1b[32mAddon written:\x1b[0m ${addonDest}`);
      console.log(`\nIn Blender: Edit > Preferences > Add-ons > Install...\nNavigate to: ${dir}\n`);
      try { execSync({ win32: `explorer "${dir}"`, darwin: `open "${dir}"` }[process.platform] || `xdg-open "${dir}"`); } catch {}
      if (!opts.auto) return;
      const os = require('os');
      const base = ({ win32: path.join(process.env.APPDATA || '', 'Blender Foundation', 'Blender'), darwin: path.join(os.homedir(), 'Library', 'Application Support', 'Blender') })[process.platform] || path.join(os.homedir(), '.config', 'blender');
      if (!fs.existsSync(base)) { console.log('Blender config dir not found:', base); return; }
      const versions = fs.readdirSync(base).filter(d => /^\d+\.\d+/.test(d));
      if (!versions.length) { console.log('No Blender version folders found in:', base); return; }
      versions.forEach(ver => {
        const dest = path.join(base, ver, 'scripts', 'addons', 'blender_bridge', '__init__.py');
        fs.mkdirSync(path.dirname(dest), { recursive: true });
        fs.copyFileSync(addonSrc, dest);
        console.log(`\x1b[32mInstalled to:\x1b[0m ${dest}`);
      });
    });
}
