...
function registerCoreCommands(program) {
  ...
  program.command('install-addon').description('Write Blender bridge addon to ./addons/blender_bridge/__init__.py')
    .action(() => {
      const src = path.join(__dirname, '..', 'addons', 'blender_bridge', '__init__.py');
      const dest = path.join(process.cwd(), 'addons', 'blender_bridge', '__init__.py');
      fs.mkdirSync(path.dirname(dest), { recursive: true });
      fs.copyFileSync(src, dest);
      const dir = path.dirname(dest);
      console.log(`\x1b[32mAddon written:\x1b[0m ${dest}`);
