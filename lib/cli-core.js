...
function blenderNotFoundError(CFG_DIR) {
  ...
  console.error(`Checked: config file, common install paths, system PATH (blender, blender4.3)`);
...
function registerCoreCommands(program) {
  const { findBlender, downloadEngine, CFG_DIR, readConfig, writeConfig } = require('./engine');
  ...
  // --- download-engine ---
  program.command('download-engine').description('Show Blender 4.3 download instructions')
